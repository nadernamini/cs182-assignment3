...
class TransformerEncoderBlock(Model):
    ...
    def call(self, inputs, self_attention_mask=None):
        ...
        # Apply the self-attention with the normalized input, use the self_attention mask as the optional
        # mask parameter.
        # Using self.self_attention
        # Step 2
        query_antecedent, memory_antecedent = norm_inputs, norm_inputs
        attn = self.self_attention((query_antecedent, memory_antecedent), mask=self_attention_mask)

        # Apply the residual connection.
        # res_attn should sum the attention output and the original, non-normalized inputs
        # Step 3
        res_attn = tf.add(attn, inputs)  # Residual connection of the attention block
...
class TransformerDecoderBlock(Model):
    ...
    def call(self, decoder_inputs, encoder_outputs, self_attention_mask=None, cross_attention_mask=None):
        ...
        query_antecedent, memory_antecedent = norm_decoder_inputs, norm_decoder_inputs
        target_selfattn = self.self_attention((query_antecedent, memory_antecedent), mask=self_attention_mask)
        res_target_self_attn = tf.add(target_selfattn, decoder_inputs)
        ...
        # This is done using self.cross_attention, and use the mask we provide: cross_attention_mask, as the optional
        # mask parameter
        query_antecedent, memory_antecedent = norm_target_selfattn, norm_encoder_outputs
        encdec_attention = self.cross_attention((query_antecedent, memory_antecedent), mask=cross_attention_mask)

        # Step 5
        # The cross-attention is followed by another residual connection
        res_encdec_attention = tf.add(encdec_attention, res_target_self_attn)

        # Step 6
        # Apply the feed-forward layer to the output of the residual of the cross_attention
...
class TransformerDecoder(Model):
    ...
    def call(self, target_input, encoder_output, encoder_mask=None, decoder_mask=None, mask_future=False,
        ...
        decoder_output = self.decoding_stack(target_embedding, encoder_outputs=encoder_output,
...
class Transformer(Model):
    ...
    def call(self, source_sequence, target_sequence, encoder_mask, decoder_mask, mask_future=True,
        ...
        # Part 1: Encode
        # Using the self.encoder, encode the source_sequence, and provide the encoder
        #_mask variable as the optional mask.
        ...
        decoder_output = self.decoder(target_sequence, encoder_output, encoder_mask=encoder_mask,
                                      decoder_mask=decoder_mask, mask_future=mask_future,
